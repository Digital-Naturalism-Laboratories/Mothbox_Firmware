#!/usr/bin/python3
"""Party.py -- PARTY mode: cycle all the lights until killed.

Entry point kept for cron / Scheduler.py / TakePhoto.py, which call this
script by path. All pin logic lives in mothbox_hw.py and dispatches on the
detected hardware (Mothbox Pro PCB or DIY relay build).
"""
import os
import sys

_HERE = os.path.dirname(os.path.abspath(__file__))
sys.path[:0] = [_HERE, os.path.dirname(_HERE)]
import mothbox_hw

import time
print("----------------- PARTY -------------------")
delay = 0.2
try:
    while True:
        mothbox_hw.flash_on();        time.sleep(delay)
        mothbox_hw.all_lights_off();  time.sleep(delay)
        mothbox_hw.attract_on();      time.sleep(delay)
        mothbox_hw.all_lights_on();   time.sleep(delay)
        mothbox_hw.all_lights_off();  time.sleep(delay)
except KeyboardInterrupt:
    mothbox_hw.all_lights_off()

